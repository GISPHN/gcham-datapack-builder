...
- Automatically add outputs to the QGIS layer tree with deterministic group ordering.
- Optionally create facility layers (P28/P05/P04/A27/P29), transport layers (P11/N07/N02), disaster layers, and background maps.
- Optionally download and process N13 road data; roads are separately selectable and disabled by default because downloads can be large.
- Create GSI designated emergency evacuation-site and designated-shelter layers, with prefecture-filtered GeoJSON fallback when direct CSV discovery is unavailable.
- Add J-SHIS 2024 probabilistic seismic-hazard information (`T30_P03_SI`); when vector conversion is unavailable, the official WMS is used at 70% opacity.
- Add OpenStreetMap Standard, GSI seamless aerial imagery, and GSI hillshade as optional background maps.
- Reuse previously downloaded source archives from the local cache.
...
Some prefectures, particularly Hokkaido, Tokyo, Kagoshima, and Okinawa, span multiple official plane-rectangular zones. Version 1.1.0 uses a prefecture-level principal zone for the combined prefecture workflow and records a warning in the execution log for these prefectures. Users working with remote islands or precision-sensitive projected measurements should verify the appropriate local zone for their intended analysis.
...
- QGISレイヤツリーへ決められたグループ順で自動追加
- `施設`：P28、P05、P04、A27、P29を必要に応じて自動取得・FGB化
- `交通`：P11、N07、N02を自動取得・FGB化
- N13道路は交通とは別に選択でき、容量が大きいため初期状態はOFF
- `災害`：国土地理院の指定緊急避難場所・指定避難所、J-SHIS 2024確率論的地震動予測地図を追加
- J-SHISのベクタ変換が利用できない場合は公式WMSを不透明度70%で使用
- `背景地図`：OpenStreetMap、地理院の全国最新写真、陰影起伏図を追加可能
- 施設・交通・災害のラベルは設定を保持したまま初期非表示
- 自治体は `N03_007` の昇順で表示
...
北海道、東京都、鹿児島県、沖縄県など、複数の平面直角座標系にまたがる都道府県については、v1.1.0では都道府県全体の処理に主要区域の系を使用し、実行ログに注意を表示します。離島等を含む高精度な距離・面積解析を行う場合は、目的地域に適した系を確認してください。

データの利用・再配布にあたっては、e-Stat、国土数値情報、国土地理院等の最新の利用条件を確認してください。詳細は [DATA_SOURCES.md](DATA_SOURCES.md) に整理しています。

## Version 1.1.0 supplemental data / 追加データ

Version 1.1.0 adds optional Facility, Transport, Disaster, and Background Map groups. Road data are selected separately and are off by default because the N13 source archives can be large. Supplemental labels are configured but hidden by default. J-SHIS probabilistic seismic hazard uses official 2024 data and may use the official WMS at 70% opacity when local vector conversion is unavailable.

バージョン1.1.0では、人口・行政区域に加えて「施設」「交通」「災害」「背景地図」を選択して作成できます。N13道路は容量が大きいため独立選択・初期OFFです。施設・交通・災害のラベルは初期非表示です。J-SHIS確率論的地震動は2024年版を使用し、ローカルベクタ化が利用できない場合は公式WMSを不透明度70%で表示します。
